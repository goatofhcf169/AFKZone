...
public class AFKManager {
     ...
     */
    public AFKManager(AFKPool plugin) {
        this.plugin = plugin;
    ...
    public AFKPlayer getOrCreateAFKPlayer(Player player) {
        return afkPlayers.computeIfAbsent(player.getUniqueId(), uuid -> {
            ...
            afkPlayer.setRewardTier(determineRewardTier(player));
            return afkPlayer;
        });
    }
    ...
    public void handleEnterRegion(Player player) {
        ...
        if (!afkPlayer.isInRegion()) {
            ...
            if (plugin.getConfigManager().isTitleEnabled()) {
                ...
                Component titleComponent = dev.alone.aFKPool.util.MessageUtil.toComponent(titleText);
                Component subtitleComponent = dev.alone.aFKPool.util.MessageUtil.toComponent(subtitleText);
            ...
            message = message.replace("%reward_interval%",
                dev.alone.aFKPool.util.MessageUtil.formatTime(plugin.getConfigManager().getRewardInterval()));

            Component messageComponent = dev.alone.aFKPool.util.MessageUtil.toComponentWithPlaceholders(player, message);
    ...
    public void updateRegionStatus(Player player, boolean inRegion) {
        ...
        // Update reward tier in case permissions changed
        String newTier = determineRewardTier(player);
        if (!newTier.equals(afkPlayer.getRewardTier())) {
            afkPlayer.setRewardTier(newTier);

            // Notify player if tier changed
            String message = plugin.getConfigManager().getMessage("tier-changed");
            String prefix = plugin.getConfigManager().getPrefix();
            message = message.replace("%prefix%", prefix)
                .replace("%reward_tier%", newTier);
            Component messageComponent = dev.alone.aFKPool.util.MessageUtil.toComponentWithPlaceholders(player, message);
            player.sendMessage(messageComponent);
        }
    }
    ...
    /**
     * Determine a player's reward tier based on permissions
     * @param player The player
     * @return The reward tier name
     */
    public String determineRewardTier(Player player) {
        List<String> tierPriority = plugin.getConfigManager().getTierPriority();

        for (String tier : tierPriority) {
            if (tier.equals("default")) {
                return "default";
            }

            // Check if player has permission explicitly (not from OP)
            // This prevents OP users from automatically getting MVP tier
            if (player.hasPermission(tier) && !tier.equals("default")) {
                // Verify it's an explicit permission, not from OP
                if (player.isPermissionSet(tier) || !player.isOp()) {
                    // Extract tier name from permission (e.g., "afkpool.vip" -> "vip")
                    String tierName = tier.contains(".") ? tier.substring(tier.lastIndexOf('.') + 1) : tier;
                    return tierName;
                }
            }
        }
