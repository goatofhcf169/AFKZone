...
public class RegionManager {
     ...
     */
    public RegionManager(AFKPool plugin) {
        this.plugin = plugin;
    ...
    /**
     * Check if a player is in the AFK region (bypassing cache for accuracy)
     * @param player The player to check
     * @return true if in AFK region AND in water
     */
    public boolean isInAFKRegion(Player player) {
        // Don't use cache for region checks to prevent stale data
        // This ensures accurate region detection when players enter/exit water
        return checkWorldGuardRegion(player);
    }

    /**
     * Check WorldGuard region directly (bypassing cache)
     * @param player The player to check
     * @return true if in AFK region AND in water
     */
    ...
    private boolean isPlayerInWater(Player player) {
        ...
        // Check the block the player is in
        if (loc.getBlock().isLiquid()) {
            return true;
        }

        // Check the block at player's eye level
        Location eyeLoc = player.getEyeLocation();
        if (eyeLoc.getBlock().isLiquid()) {
            return true;
        }

        // Check if player is considered to be in water by Bukkit
